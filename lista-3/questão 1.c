...
float calcularFrequencia(int frequencia) {
    
    return frequencia * 100.0;
}
...
void cadastrarAluno(struct Aluno *aluno) {
    ...
    aluno->frequencia = calcularFrequencia(aluno->frequencia);
    if(aluno->frequencia <=60){
        printf("Aluno reprovado por frequência!\n");

	}
    if(aluno->frequencia >=60){
        printf("Aluno aprovado por frequência");
    }
}
...
void consultarAlunoPorMatricula(struct Aluno alunos[], int numAlunos, int matricula) {
    ...
    if (!alunoEncontrado) {
        printf("Mátricula não cadastrada %d.\n", matricula);
    }
}
